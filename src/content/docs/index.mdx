...
#### package.json
Link `fcore` and configure the zero-config build runner:
...
```json
{
  ...
  "scripts": {
    ...
    "format": "prettier --write --log-level warn \"src/**/*.{ts,tsx}\""
  },
  "dependencies": {
    "fcore": "file:../fcore"
  },
...
#### tsconfig.json
Extend `fcore/tsconfig.base.json` to inherit all Factorio 2.0 compiler settings, JSX factories, AST style transformers, and Lua import plugins in a single line:

```json
{
  "extends": "fcore/tsconfig.base.json",
  "compilerOptions": {
    "rootDir": "./src",
    "outDir": "./dist"
  },
...
#### eslint.config.mjs
One-line ESLint 9 configuration enforcing Factorio Lua 5.2 VM compatibility and performance best practices:
...
<CardGrid stagger>
  ...
  <Card title="Infrastructure & Tooling" icon="setting">
    Zero-config CLI runners (`fcore-build`, `fcore-release`), ESLint 9 VM guardrails, TSTL compiler plugins, and automated mod packaging. [Explore Infrastructure &rarr;](/fcore/infrastructure/)
  </Card>
</CardGrid>
